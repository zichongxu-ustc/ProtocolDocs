...
6.  干浴器选择 <b>Prog. A</b>，然后手动将盖子温度按照以下操作设置为 +5.0  
a.  选择 <b>Edit</b>  
b.  选择 <b>LidMode</b> 切换到+5.0  
c.  选择 <b>Save/Return</b>  
1.  选择“<b>Run</b>”，将干浴器预热至适用裂解程序的保持温度。 
2.  选择“<b>Lid On</b>”预热干浴器盖子，当控制盖子温度时显示的是 <b>LidOff</b> 
### 实验步骤
<mark>一定要缓慢加入试剂以免出现气泡</mark>

1.  向 CPIP 管中的 CPIP 层缓慢加入 1 μL RNI（40 units）   
2.  <span style="color: blue;">使用 200 μL 的大口径移液器吸头，缓慢移液 10 次以混合细胞悬液。 </span>
...
b.  等待 5 s。  
c.  抽吸下层液相，直至 CPIP 乳液底部接近管架 B 层，<mark>注意不要抽吸到任何乳液。</mark>  
d.  移除针头时，在管子内壁擦拭一下。  
e.  弃去针头和注射器到利器盒中  
...
5.  轻弹管壁使CPIP pellet分散
6.  涡旋 5s 混匀 
7.  瞬时离心 
8.  将 CPIP 管置于干浴器中 
9.  选择 Skip，然后选择 Yes 运行 cDNA 合成程序 
<mark><b>注：当干浴器温度达到 25℃时移去样品</b></mark> 

10.  从干浴器中移去样品 
11.   干浴器选择 Prog.F，然后手动将盖子温度按照以下操作设置为 +5.0。   
a.  选择 Edit   
b.  选择 LidMode 切换到+5.0   
c.  选择 Save/Return   
12.   选择“Lid On”预热干浴器盖子，当控制盖子温度时显示的是 LidOff 
13.   选择“Run”，将干浴器预热至 37℃ 
14.   每个反应管加 20 μL RTF 
15.   涡旋混匀 
16.   若需要可瞬时离心收集管壁液体 
17.   将样品放入干浴器 
18.   选择 Skip，然后选择 Yes 运行程序 
19.   当程序运行完成后立即准备 CPIP 清洗 
...
- PWB1（660 μL） 
- 无核酸酶水（660 μL）   
每个样品使用 1320 μL 0.5 × PWB1
### 实验步骤
1.  从干浴器中取出样品 
2.  离心 CPIP 管 1 min
3.  不触及 CPIP 的情况下弃去 130 μL 上清 
4.  按以下步骤清洗 CPIPs：    
a.  加入 450 μL 0.5 × PWB1   
b.  涡旋 5 s。   
c.  离心 1 min 后置于管架上   
d.  不要触及沉淀，弃去上清至管架 A 刻度线处。   
1.  重复步骤 3 第二次清洗 PIPs。使用 350 μL 0.5 × PWB1 
2.  重复步骤 3 第三次清洗 PIPs。使用 350 μL 0.5 × PWB1 
3.  每次处理两个反应，离心 1min。
4.  保持管子的角度，立即弃去上清至 0.1 ml 标记处 
5.  冰上放置样品并<mark>立即进行 cDNA 扩增</mark> 
## Step 7 cDNA 扩增（1 hr） 
### 耗材准备 
...
- PMM4——吹吸混匀后瞬时离心 
- WTA2——吹吸混匀后瞬时离心 
1.  在 PCR 仪中设置<b> cDNA Amplication </b>程序： 
Set the lid temperature to 105 ℃ 
Set the reaction volume to 68 μL 
...
<table>
  ...
  <tr>
    <td>PMM4 </td>
    <td>4X PCR Master Mix </td>
    <td>illumina Single Cell 3’ RNA Capture, T2, Box 4 </td>
    <td>冰上放置 </td>
  </tr>
  <tr>
    <td>WTA2</td>
...
11.  如果存在剩余的 CPIP，转移上清到新离心管中，测量转移上清的体积，从而调整后续清洗过程的磁珠添加量 
...
| Reagent | Volume Per Reaction(μL) | 4.4X Reaction Master Mix(μL) | 8.8X Reaction Master Mix(μL) |
| :-----: | :---------------------: | :--------------------------: | :--------------------------: |
|  PMM4   |          33.8           |             148.7              |             297.4              |
|  CAPM   |           1.4           |              6.2               |              12.3              |
|  Total  |          35.2           |             154.9              |             309.7              |
2.  涡旋 10s，然后瞬时离心 
